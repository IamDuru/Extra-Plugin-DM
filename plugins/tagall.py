...
@app.on_message(
    filters.command(["l", "allmention", "mentionall",], prefixes=["/", "!", "%", ",", "-", ".", "@", "#", "a", "A"])
)
async def tag_all_users(_, message):
    ...
    if len(message.command) < 2 and not replied:
        await message.reply_text(
            "** ɢɪᴠᴇ sᴏᴍᴇ ᴛᴇxᴛ ᴛᴏ ᴛᴀɢ ᴀʟʟ, ʟɪᴋᴇ »** `al Hi Friends`"
        )
        return
    if replied:
        usernum = 0
...
@app.on_message(filters.command(["utag", "ll", "ention"], prefixes=["a", "A", "m", "M"]))
async def tag_all_users(_,message): 
    replied = message.reply_to_message  
    if len(message.command) < 2 and not replied:
